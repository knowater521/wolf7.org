...
log_file='./access_log'
now_time='20/Dec/2017:03:33'
project='example'
email_add='deniss.wang@example.com'
statuscode_list=(404 401 403 400 500 501 503 504)
web_filter_list='robot.txt|favicon.ico|testproxy.php|device_description.xml|location.default.css'
...
web_code_sendemail(){
    ...
    do
        ...
        check_dir_list=`tail -n 50 $log_file|grep -w $dir_list|grep -w $code_list|egrep -v $web_filter_list|awk 'END {print}'|awk '{print $8}' |awk -F'/' '{print $2}'`
            ...
            #if [[ $statuscode == $code_list &&  $status_time == $now_time ]];then
            # print logs detail send to email
